...
// ✅ No Old Page Flash on Switch
// ✅ Mobile Google Login Fixed
// ✅ Font Change Working
// ✅ Social Media Redirect Support
// ============================================
...
function showLoading(msg = "লোড হচ্ছে...") {
    hideLoading(); // Clear existing first
    ...
    // Auto hide after 6 seconds safety
...
// ============================================
// ✅ AUTH & LOGIN FUNCTIONS (FIXED NO FLASH)
// ============================================
...
function guestLogin() {
    ...
    // ✅ Clear State Immediately to prevent flash
    currentUser = null;
    ...
    auth.signOut().then(() => {
        ...
        currentUser = userCredential.user;        currentUserRole = 'guest';
...
function googleLogin() {
    ...
    auth.signOut().then(() => {
        // Check mobile for redirect vs popup
        ...
        // Errors for Popup or Redirect handled here mostly
        if (error.code !== 'auth/popup-closed-by-user') {
             console.error("❌ Google login error:", error);             // Don't hide loading immediately if redirect happened
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Handle Redirect Result (Mobile)    auth.getRedirectResult().then((result) => {
        if (result.user) {
    ...
    auth.onAuthStateChanged((user) => {
        if (user) {
            // Only load user if we don't have a role yet (prevents double loading)
            if (!currentUserRole) {
                loadUser(user.uid);
            }
        } else {
...
function logout() {
    ...
    auth.signOut().then(() => {
        ...
        hideLoading();
        showPage('loginPage');
        // Optional: Force reload to clear any stubborn cache
...
function getStyle(id, prop) {    const el = document.getElementById(id);
...
function submitReview() {
    if (currentUserRole !== 'tutor' && currentUserRole !== 'guardian') { alert("শুধুমাত্র টিউটর এবং অভিভাবক রিভিউ দিতে পারবেন"); return; }    const t = document.getElementById('reviewText').value;
